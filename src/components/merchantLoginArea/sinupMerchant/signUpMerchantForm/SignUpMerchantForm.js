...
function SignUpMerchantForm({ initalValue, handleChange, mobileGenerateOtpMobile, userIDD, mobileVeridedInput, getOtp, mobileOtp, emailVeridedInput, emailgetOtp, nextForm, emailOtp, submitOtpEmail, formResiter, setEmailVeridedInput, setMobileOtp, setMobileVerified, handleCountryCode, countryCode, setEmailOtp, setFormResiter, loader1, showMobileOtp, handleClose2, mobileExit, setMobileExit, veriFiedIconMobile, setVeriFiedIconMobile
    ...
    // console.log('sinup', param);
    const extractedId = param?.id?.split("=")[0] || "";
    const extractedPosition = param?.id?.split("=")[1] || "left";
    ...
    const [resiter, setResister] = useState({
        ...
        password: '',
        repassword: '',
        currency_id: '678e20f154cd8162ac153c2d',
        position: '',
        sponsor_id: '',
        refer_id: ''
    })
    const [referDataInitial, setReferDataInitial] = useState({
        refer_id: param?.id && param?.id ? extractedId : '',
        position: param?.id && param?.id ? extractedPosition : 'left'
    })
    ...
    const referOnchange = (e) => {
        ...
        const clone = { ...referDataInitial, [name]: value };
    ...
    useEffect(() => {
        if (param.id && extractedId.length >= 9) {
            getRefer({ refer_id: extractedId, position: extractedPosition });
        }
    }, [param.id, extractedId, extractedPosition]);
            ...
            <div className="mt-3">
                ...
                {formResiter ? <form >
                    <div className="row">
                        ...
                        <div className="col-lg-6 col-md-6 col-sm-12 mb-3 form-group uk-scrollspy-inview uk-animation-slide-bottom">
                            ...
                            <div className="input-group mb-1">
                                ...
                                >
                                    ...
                                    {/* <option value="11111" >Delhi</option> */}
                                    {stateOptions
                        ...
                        {loadingRef && <div>Loading...</div>}
                        {!loadingRef && referData && (
                            <div className="col-lg-6">
                                <div className="refer-details">
                                    <div className="mr-1">
                                        <span style={{ color: 'red' }}>{referData?.data?.name}</span>
                                    </div>
                                    {/* <div>
                        ...
                        <div>
                            <button type="button"
                                ...
                                //     : 'btn-login'
                                //     }`}
                                className={`btn disableebuttoncls'
                                    btn-login
                                    `}
